...
impl<'a> DockerGenerator<'a> {
    ...
    pub fn generate(&self, out: &Path) -> Result<()> {
        // Root artifacts (lifecycle of the whole app)
        ...
        for feature in self.features {
            if !feature.docker_services.is_empty() {
                ...
                let compose_content = self.render_component_compose(feature);
                std::fs::write(feat_dir.join("docker-compose.yml"), compose_content)?;
            }
        }
    ...
    fn render_dockerfile(&self) -> String {
        ...
        format!(
            r#"# ─────────────────────────────────────────────────────────
# Multi-stage Dockerfile — generated by SpringbootGen
# ─────────────────────────────────────────────────────────
...
{volumes_section}"#,
    ...
    fn render_component_compose(&self, feature: &FeatureSpec) -> String {
        let mut services = String::new();
        let mut volumes: Vec<String> = Vec::new();
        ...
        for svc in feature.docker_services {
            ...
            for vol in svc.volumes {
                if let Some(vol_name) = vol.split(':').next() {
                    if !vol_name.starts_with('.') && !vol_name.starts_with('/') {
                        let vol_key = vol_name.to_string();
                        if !volumes.contains(&vol_key) {
                            volumes.push(vol_key);
                        }
                    }
                }
            }
