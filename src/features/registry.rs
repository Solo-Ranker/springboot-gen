...
pub fn all_features() -> Vec<FeatureSpec> {
    vec![
        ...
        feature!(
            ...
            services:  [svc!("redis", "redis:7.2-alpine",
                ports:   &["6380:6380"],
                volumes: &["redis_data:/data", "./docker/redis-ssl/certs:/tls:ro"],
            ...
            conflicts: ["redis", "redis-sentinel", "redis-ssl-sentinel"],
        ),
        feature!(
            key:         "redis-sentinel",
            ...
            conflicts: ["redis", "redis-ssl", "redis-ssl-sentinel"],
        ),
        feature!(
            key:         "redis-ssl-sentinel",
            name:        "Redis (SSL/TLS + Sentinel HA)",
            description: "Redis with TLS encryption AND Sentinel high-availability failover",
            deps: [
                dep!("org.springframework.boot", "spring-boot-starter-data-redis"),
                dep!("io.lettuce", "lettuce-core"),
                dep!("org.apache.commons", "commons-pool2"),
            ...
            services:  [
                svc!("redis-master", "redis:7.2-alpine",
                    ports:   &["6379:6379"],
                    volumes: &["redis_master_data:/data", "./docker/redis-ssl/certs:/tls:ro"],
                    health:  "redis-cli --tls ping",
                ),
                svc!("redis-replica-1", "redis:7.2-alpine",
                    ports:   &["6380:6379"],
                    volumes: &["redis_replica1_data:/data", "./docker/redis-ssl/certs:/tls:ro"],
                    health:  "redis-cli --tls ping",
                    depends: &["redis-master"],
                ),
                svc!("redis-sentinel-1", "redis:7.2-alpine",
                    ports:   &["26379:26379"],
                    volumes: &["./docker/redis-ssl/certs:/tls:ro"],
                    health:  "redis-cli --tls -p 26379 ping",
                    depends: &["redis-master", "redis-replica-1"],
                ),
                svc!("redis-sentinel-2", "redis:7.2-alpine",
                    ports:   &["26380:26379"],
                    volumes: &["./docker/redis-ssl/certs:/tls:ro"],
                    health:  "redis-cli --tls -p 26379 ping",
                    depends: &["redis-master", "redis-replica-1"],
                ),
                svc!("redis-sentinel-3", "redis:7.2-alpine",
                    ports:   &["26381:26379"],
                    volumes: &["./docker/redis-ssl/certs:/tls:ro"],
                    health:  "redis-cli --tls -p 26379 ping",
                    depends: &["redis-master", "redis-replica-1"],
            ...
            java:      ["RedisSslSentinelConfig", "CacheConfig"],
            conflicts: ["redis", "redis-ssl", "redis-sentinel"],
        ),
        feature!(
            key:         "kafka",
            name:        "Apache Kafka",
        ...
        feature!(
            ...
            java:      ["JpaConfig"],
            conflicts: ["mysql", "postgres-ssl", "mysql-ssl"],
        ),
        feature!(
            key:         "postgres-ssl",
            name:        "PostgreSQL (SSL/TLS)",
            description: "PostgreSQL with SSL client certificate authentication",
            deps: [
                dep!("org.springframework.boot", "spring-boot-starter-data-jpa"),
                dep!("org.postgresql", "postgresql"),
                dep!("org.flywaydb", "flyway-core"),
            ...
            env: [
                ...
                "DB_POOL_MAX" => "10",
                "DB_SSL_MODE" => "require",
                "DB_SSL_KEYSTORE" => "classpath:ssl/db-client.p12",
                "DB_SSL_KEYSTORE_PASSWORD" => "changeit",
                "DB_SSL_TRUSTSTORE" => "classpath:ssl/db-truststore.p12",
                "DB_SSL_TRUSTSTORE_PASSWORD" => "changeit",
            ],
            java:      ["JpaConfig", "PostgresSslConfig"],
            conflicts: ["postgres", "mysql", "mysql-ssl"],
        ),
        feature!(
            key:         "mysql",
            name:        "MySQL",
            ...
            java:      ["JpaConfig"],
            conflicts: ["postgres", "mysql-ssl", "postgres-ssl"],
        ),
        feature!(
            key:         "mysql-ssl",
            name:        "MySQL (SSL/TLS)",
            description: "MySQL with SSL encrypted connections",
            deps: [
                dep!("org.springframework.boot", "spring-boot-starter-data-jpa"),
                dep!("com.mysql", "mysql-connector-j"),
                dep!("org.flywaydb", "flyway-mysql"),
            ...
            services:  [svc!("mysql", "mysql:8.3",
                ...
                volumes: &[
                    ...
                    "./docker/mysql-ssl/certs:/etc/mysql/certs:ro",
                ],
                health:  "mysqladmin ping -h localhost",
            )],
            env: [
            ...
            java:      ["JpaConfig", "MysqlSslConfig"],
            conflicts: ["mysql", "postgres", "postgres-ssl"],
        ),
        feature!(
            key:         "mongodb",
            name:        "MongoDB",
